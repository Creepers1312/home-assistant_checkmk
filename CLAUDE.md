...
- **Device linking via MAC** (`parsing.extract_macs` + `__init__._async_register_devices`): host devices are registered with `connections={(CONNECTION_NETWORK_MAC, mac)}` so HA's device registry merges them with the same physical device shown by other integrations (FRITZ!Box Tools, UniFi, Hue Bridge, …). The MACs are *not* fetched from a separate endpoint - they're parsed out of the existing `plugin_output` of `lnx_if` / `winperf_if` interface checks (`MAC: aa:bb:cc:dd:ee:ff`, colon or hyphen, lowercased and normalised). Means: only hosts with active interface monitoring get linked; pure ping-/HTTP-only hosts stay standalone, and that's the right behaviour. The all-zero MAC is dropped to avoid linking unrelated virtual NICs. Devices are upserted via `device_registry.async_get_or_create` on every coordinator update, so MACs that appear in a later poll heal themselves; HA never removes connections though, so a NIC replacement leaves the old MAC behind until the device is removed and re-added.
- **Metric-sensor visibility tiers** (`metrics.visibility_for(name)`): perfdata can still explode into hundreds of values per host, so the per-metric sensors are split into:
  ...
  - **Diagnostic visible** (`EntityCategory.DIAGNOSTIC`, enabled): a small, deliberately short list of secondary signals that the primary tier does not cover - `disk_read_throughput`, `disk_write_throughput`, `disk_latency`, `disk_utilization`, `mem_available`, `swap_used`, `pagefile_used_percent`. Anything redundant with the primary tier (e.g. `mem_free`/`mem_total` when `mem_used` is already primary) lives in the hidden tier.
